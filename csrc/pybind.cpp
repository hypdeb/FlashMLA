 ...
 ******************************************************************************/
 ...
 fwd_kvcache_mla(
     ...
     TORCH_CHECK(head_size_k == 576 || head_size_k == 320, "Only head_size_k == 576 or head_size_k == 320 is supported");
     TORCH_CHECK(head_size_v == 512 || head_size_v == 256, "Only head_size_v == 512 or head_size_v == 256 is supported");
     ...
     auto stream = at::cuda::getCurrentCUDAStream().stream();
     TORCH_CHECK(head_size_k == 576 || head_size_k == 320);
             ...
             if (is_fp8) {
                 ...
                 if (q_dtype == torch::kBFloat16) {
                     if (head_size_k == 576 && head_size_v == 512) {
                         sm90::run_flash_splitkv_mla_kernel<cutlass::bfloat16_t, 576, 512>(params, stream);
                     } else if (head_size_k == 320 && head_size_v == 256) {
                         sm90::run_flash_splitkv_mla_kernel<cutlass::bfloat16_t, 320, 256>(params, stream);
                     } else {
                     ...
                     if (head_size_k == 576 && head_size_v == 512) {
                         sm90::run_flash_splitkv_mla_kernel<cutlass::half_t, 576, 512>(params, stream);
                     } else if (head_size_k == 320 && head_size_v == 256) {
                         sm90::run_flash_splitkv_mla_kernel<cutlass::half_t, 320, 256>(params, stream);
                     } else {
                         TORCH_CHECK(false, "Unsupported FlashMLA configuration");
                     }
 #endif
